...
void messaging_service::register_read_data(std::function<future<rpc::tuple<foreign_ptr<lw_shared_ptr<query::result>>, cache_temperature, int64_t>> (const rpc::client_info&, rpc::opt_time_point t, query::read_command cmd, ::compat::wrapping_partition_range pr, rpc::optional<query::digest_algorithm> oda)>&& func) {
...
future<rpc::tuple<query::result, rpc::optional<cache_temperature>, int64_t>> messaging_service::send_read_data(msg_addr id, clock_type::time_point timeout, const query::read_command& cmd, const dht::partition_range& pr, query::digest_algorithm da) {
    return send_message_timeout<future<rpc::tuple<query::result, rpc::optional<cache_temperature>, int64_t>>>(this, messaging_verb::READ_DATA, std::move(id), timeout, cmd, pr, da);
}
